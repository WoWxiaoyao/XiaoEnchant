        ...
        if(i != null)
        {
            if(c.getIntegerList("List").contains(i.getId()))
            {
                if((ItemUtil.TakePlayerItem(p,ItemUtil.Intensify(1))) || (VaultUtil.reduceMoney(p,NeedMoney)))
                {
                    ...
                    PrintUtil.PrintCommandSender(p,false, Lang.Intensify_MoneyNotEnough.replace("{money_need}",String.valueOf(NeedMoney)).replace("{money}",String.valueOf(VaultUtil.getMoney(p))));
                }
            }
        }
    }
