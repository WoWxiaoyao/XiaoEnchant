...
{
    public static FormWindowSimple IntensifyUi(Player p,int i)
    {
        ...
        for (String s : c.getKeys(false))
        {
            ...
            if(ss.length == 2)
            {
                ...
                if(pattern.matcher(ss[1]).matches())
                {
                    ...
                    if( (Ench>=0) && (Ench <= 32))
                    {
                        if(FileUtil.intensify.getIntegerList("IntensifyList."+s+".List").contains(i))
                        {
    ...
    public static FormWindowSimple startIntensifyUi(Player p, int ench)
    {
        ...
        if((item == null) || (item.getId() <= 0))
        {
            ...
            ui = new FormWindowSimple(PrintUtil.cc("&4&l附魔强化 &7- &c&l"+EnchUtil.getEnchName(ench)),PrintUtil.cc(Content.toString()
                    ...
                    .replace("{money}", String.valueOf(VaultUtil.getMoney(p)))
